...
def save_config(cfg: dict) -> bool:
    ...
    try:
        ...
        log(f"!! CANNOT WRITE {os.path.dirname(CONFIG_PATH)} ({e.strerror}). The "
            "mounted config folder isn't writable by this container — on Unraid run "
            "`chmod -R 777 /mnt/user/appdata/owlet/config` and restart. Settings "
...
if __name__ == "__main__":
    # Seed defaults only when nothing exists yet — never overwrite an existing
    # owlet.env (it may already hold the camera creds from a prior diagnose).
    if not os.path.exists(CONFIG_PATH) and not os.path.exists(ENV_PATH):
        if not save_config(dict(DEFAULTS)):
