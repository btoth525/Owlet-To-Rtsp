...
if __name__ == "__main__":
    ...
    # Priority: persistent cert in /config/ssl/ (generated by start-bionic.sh
    # via openssl) → pyopenssl 'adhoc' (auto-generated, changes each restart)
    # → plain HTTP fallback (mic blocked, but UI still works).
    cert = "/config/ssl/cert.pem"
    key  = "/config/ssl/key.pem"
    if os.path.isfile(cert) and os.path.isfile(key):
        ssl_ctx = (cert, key)
        log("TLS cert found — control panel starting on HTTPS (persistent cert)")
    else:
        try:
            import OpenSSL  # noqa: F401
            ssl_ctx = "adhoc"
            log("TLS: using pyopenssl adhoc cert — control panel starting on HTTPS")
        except ImportError:
            ssl_ctx = None
            log("WARNING: no TLS cert and pyopenssl not installed — mic access "
                "will be blocked by the browser (needs https or localhost)")
